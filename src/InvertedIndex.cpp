...
void InvertedIndex::UpdateDocumentBase(std::vector<std::string> input_docs) {
    ...
    while (required_threads > 0) {
        threads_packs.push_back(
                (required_threads / one_time_threads_max > 0) ? one_time_threads_max : required_threads);
    ...
    //// @threads start packets of threads for loading docs � freq_dictionary's keys
    ...
    for (auto key_it = begin(freq_dictionary); key_it != end(freq_dictionary); ++key_it) {
        ...
        for (entry.docId = 0; entry.docId < docs.size(); ++entry.docId) {
            std::stringstream DATA(docs[entry.docId]);
            std::string word;
            entry.count = 0;
