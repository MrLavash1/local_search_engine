...
std::vector<std::string> ConverterJSON::GetTextDocuments(const std::string &fileName)
{
    ...
    auto DOCS_BASE_LOAD = [&](const size_t& doc_id) {
        ...
        while (text_FILE >> word) {
            if (word_counter++ > 1000) throw Text_ExceptionALotOf();
            if (word.length() > 100) throw Word_Exception100Chars();
            text += word + " ";
        }
...
std::vector<std::string> ConverterJSON::GetRequests()
{
    ...
    if (json_data["requests"].size() > 1000) throw MoreThen1000Requests_Exception();
    ...
    for (std::string request : json_data["requests"]) {
        ...
        if (word_counter > 10) throw WordsRequests_Exception();
    }
    ...
    for (int answer_number = 0; answer_number < answers.size(); ++answer_number) {
        ...
        if (answers[answer_number].empty())
            ...
            if (answers[answer_number].size() == 1)
                json_data["answers"][request_name] = {{"docid", answers[answer_number][0].docId }, {"rank", answers[answer_number][0].rank } };
            else
                for (auto element : answers[answer_number])
                    json_data["answers"][request_name]["relevance"].push_back("docid: " + std::to_string(element.docId) + ", rank:" + std::to_string(element.rank));
        }
    }
